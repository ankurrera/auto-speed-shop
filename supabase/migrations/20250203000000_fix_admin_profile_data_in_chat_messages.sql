...
-- This ensures admin messages show the admin's name/email instead of "Unknown User"

-- Update the function to handle both user and admin profile data
...
BEGIN
  ...
  IF NEW.is_from_admin = true AND NEW.admin_id IS NOT NULL THEN
    -- For admin messages, populate with admin's profile data
    SELECT 
  ...
  ELSE
    -- For user messages, populate with user's profile data
    SELECT 
      COALESCE(p.first_name, '') as first_name,
      COALESCE(p.last_name, '') as last_name,
      COALESCE(p.email, '') as email
...
-- This fixes any existing admin messages that currently show "Unknown User"
...
WHERE chat_messages.is_from_admin = true 
  ...
  AND (chat_messages.first_name = '' OR chat_messages.first_name IS NULL OR 
       chat_messages.last_name = '' OR chat_messages.last_name IS NULL OR
       chat_messages.email = '' OR chat_messages.email IS NULL);