...
const ProductCard = ({
  ...
  // Default handler (fallback if no onAddToCart prop is passed)
  ...
  const displayImage =
    image_urls && image_urls.length > 0 ? image_urls[currentImageIndex] : "/placeholder.svg";
  ...
  const getDominantColor = (imgSrc: string) => {
    ...
    img.onload = () => {
      ...
      setDominantColor(`rgb(${pixelData[0]}, ${pixelData[1]}, ${pixelData[2]})`);
    };
    img.onerror = () => setDominantColor("bg-muted");
    img.src = imgSrc;
  };
    ...
    <Link to={`/products/${id}`} className={cn("block", className)}>
      <Card className={cn("group hover:shadow-md transition-all duration-300 border-border")}>
        ...
        <CardFooter className="p-4 pt-0">
          <Button
            className="w-full"
            onClick={onAddToCart ? onAddToCart : handleAddToCart}  // ✅ support both
            disabled={!inStock}
          >
