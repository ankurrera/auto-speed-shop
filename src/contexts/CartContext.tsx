  ...
  const fetchCartItems = useCallback(async (currentUserId: string | null) => {
    ...
    try {
      ...
      const productsItems: CartItem[] = (productsData || []).map((item) => ({
        ...
        image: item.products.image_urls && item.products.image_urls.length > 0 ? item.products.image_urls[0] : '/placeholder.svg',
      ...
      const partsItems: CartItem[] = (partsData || []).map((item) => ({
        ...
        image: item.parts.image_urls && item.parts.image_urls.length > 0 ? item.parts.image_urls[0] : '/placeholder.svg',
        quantity: item.quantity,
        is_part: true,
