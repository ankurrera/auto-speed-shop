    ...
    const conversationsWithMessages = await Promise.all(
      uniqueUserIds.map(async (userId: string) => {
        try {
          ...
          if (finalUserProfile?.is_admin === true) {
            console.log('[ChatService] Skipping admin profile:', userId);
            return null;
          }

          // Use actual profile or fallback
          const finalProfile = finalUserProfile ? {
            first_name: finalUserProfile.first_name || 'Unknown',
            last_name: finalUserProfile.last_name || 'User', 
            email: finalUserProfile.email || `user-${userId.slice(0, 8)}@unknown.com`
          } : {
            first_name: 'Unknown',
            last_name: 'User',
            email: `user-${userId.slice(0, 8)}@unknown.com`
